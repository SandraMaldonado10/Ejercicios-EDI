...
void PilaInformes::insertarInforme(Informe* in) { //el informe que tiene por parámetros lo mete dentro de la pila en la parte de arriba 
...
void PilaInformes::mostrarInformesMedico(string apellidos) {
	...
	while (!this->pInformes->estaVacia()) {  // miro si no está vacía
		in = this->pInformes->getCima(); // lo selecciono en la cima
		m = in->getMed(); // se obtiene el médico
		if (m->getApellidosMedico() == apellidos) { //miro si coinciden los apellidos del médico, con el que quiero
